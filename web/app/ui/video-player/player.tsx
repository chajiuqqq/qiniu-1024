...
const PlyrComponent: React.FC<opts>= ({v,onLike,onCollect,onCancelLike,onCancelCollect}) => {
  ...
  return (
    <div className="relative max-w-screen">
      ...
      <div className="absolute bottom-10 left-5 bg-black/50 rounded-lg p-5 text-white w-64">
        ...
        <button className="border rounded-lg border-white px-4 py-1 ml-5 hover:border-sky-400 hover:text-sky-400"> + 关注</button>
        <div className="pt-2">{v.play_count} 播放 | {v.description}</div>
      </div>
      <div className="absolute bottom-10 right-2 p-4 flex flex-col justify-center items-center space-y-2 bg-black/50 rounded-lg">
        {
          v.liked ? (
            <SolidHeartIcon className="w-6 text-pink-500"  onClick={onCancelLike}></SolidHeartIcon>
          ) : (
        ...
        {
          v.collected?(
            <SolidStarIcon className="w-6 text-yellow-500" onClick={onCancelCollect}></SolidStarIcon>
          ):(
